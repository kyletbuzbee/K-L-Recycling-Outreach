...
class CRMBLUEPRINT_ANALYZER:
    ...
    def load_configurations(self):
        ...
        print(f"   ✅ Loaded {len(self.schema_columns)} schema objects")
        print(f"   ✅ Loaded {len(self.valid_outcomes)} valid outcomes")
        print(f"   ✅ Loaded {len(self.valid_stages)} valid stages")
        print(f"   ✅ Loaded {len(self.workflow_rules)} workflow rules")
    ...
    def _load_schema_from_config(self, path: Path):
        ...
        try:
            ...
            if headers_match:
                ...
                print(f"   ✅ Successfully parsed Config.js: Found {len(self.schema_columns)} sheets")
            else:
                print(f"   ⚠️ Could not find HEADERS definition in {path.name}")

        except Exception as e:
            print(f"   ❌ Error loading schema from Config.js: {e}")
    ...
    def parse_all_files(self):
        ...
        # REMOVED 'csv' from exclusions to ensure Settings.csv is found for validation
        exclude_dirs = {'.git', 'node_modules', '__pycache__', '.idea', 'venv', 'plans', 'ai_logic_review'}
        ...
        print(f"   ✅ Parsed {len(self.files)} files")
        ...
        if cache_file.exists():
            try:
                ...
                if cached_data['hash'] == current_hash:
                    ...
                    self.stats['total_lines'] += cached_data['analysis'].lines_of_code
                    self.stats['total_functions'] += len(cached_data['analysis'].functions)
                    return
            except Exception:
    ...
    def analyze_dependencies(self):
        ...
        orphaned = []
        for full_name, func_info in self.functions.items():
            if not func_info.called_by and func_info.name not in ['onOpen', 'doGet', 'onFormSubmit', 'onEdit']:
                orphaned.append(func_info)
        
        for func in orphaned:
            if func.complexity_score > 5:  # Complex orphaned function is a concern
                self._add_issue(
                    func.file, func.line_start,
                    IssueSeverity.MEDIUM, IssueCategory.MAINTAINABILITY,
                    f"Function '{func.name}' appears to be orphaned (never called) but has high complexity",
                    f"Function defined at line {func.line_start}",
                    "Verify if this function is needed or remove if dead code"
                )
        ...
        for rel_path, file_analysis in self.files.items():
            ...
            try:
                ...
                for func in file_analysis.functions:
                    ...
                    if not func.has_error_handling and func.complexity_score > 5:
                        self._add_issue(
                            rel_path, func.line_start,
                            IssueSeverity.MEDIUM, IssueCategory.ERROR_HANDLING,
                            f"Function '{func.name}' lacks error handling",
                            f"Complex function without try/catch",
                            "Add try/catch blocks for robust error handling"
                        )

                    # Check for missing logging
                    if not func.has_logging and len(func.calls) > 3:
                        self._add_issue(
                            rel_path, func.line_start,
                            IssueSeverity.LOW, IssueCategory.CODE_QUALITY,
                            f"Function '{func.name}' has no logging",
                            f"Function with multiple calls should log",
    ...
    def analyze_security(self):
        ...
        for rel_path, file_analysis in self.files.items():
            ...
            try:
                ...
                print(f"   ⚠️ Error analyzing security for {rel_path}: {e}")
    ...
    def analyze_performance(self):
        ...
        performance_patterns = [
            (r'getDataRange\(\)\.getValues\(\)', "HIGH", 
             "Loading entire sheet into memory - consider batching for large datasets"),
            (r'for\s*\([^)]+\)\s*\{[^}]*getRange\(', "HIGH",
             "Loop with getRange calls - very slow, batch operations instead"),
            (r'appendRow\([^)]+\)', "MEDIUM",
             "appendRow in loop is slow - batch append operations"),
            (r'SpreadsheetApp\.flush\(\)', "INFO",
             "SpreadsheetApp.flush() can impact performance"),
        ]
        ...
        for rel_path, file_analysis in self.files.items():
            ...
            try:
                ...
                print(f"   ⚠️ Error analyzing performance for {rel_path}: {e}")
